package com.jlumart.interceptor;
...
@Component
@Slf4j
public class JwtTokenCourierInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        //1、从请求头中获取令牌
        String token = request.getHeader(jwtProperties.getCourierTokenName());

        //2、校验令牌
        try {
            log.info("jwt校验:{}", token);
            Claims claims = JwtUtil.parseJWT(jwtProperties.getCourierSecretKey(), token);
            Long Id = Long.valueOf(claims.get(JwtClaimsConstant.COURIER_ID).toString());
            // 在redis中进行校验
